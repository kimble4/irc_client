...
void ircSetServer(const char * server, int port);
...
boolean ircParseInput(boolean buffer_overflow);

void ircJoinChannelChannel(const __FlashStringHelper *channel);
...
void ircSendNoticeNotice(const char *target, const __FlashStringHelper *message);
